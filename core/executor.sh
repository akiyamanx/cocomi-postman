...
# === プロジェクトリポジトリのgit push ===
# v2.2修正 - 戻り値: 0=push成功, 1=push失敗, 2=変更なし
git_push_project() {
    local REPO_PATH=$1
    local COMMIT_MSG=$2
...
run_single_mission() {
    ...
    # v3.0追加 - ステップパターン指示書判定（条件分岐＋エスカレーション）
    if has_steps "$MISSION_FILE"; then
        if has_step_pattern "$MISSION_FILE"; then
            echo -e "  ${CYAN}📋 ステップパターン指示書を検出！条件分岐モードに切り替えます${NC}"
            # shellcheck source=core/step-pattern.sh
            source "$POSTMAN_DIR/core/step-pattern.sh"
            run_step_pattern_mission "$MISSION_FILE" "$MISSION_NAME"
            return $?
        fi
