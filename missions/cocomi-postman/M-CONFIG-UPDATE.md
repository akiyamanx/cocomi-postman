...
# M-CONFIG-UPDATE: config.example.json更新 + CLAUDE.md v1.1更新

**ミッションタイプ:** maintenance
...
config.example.jsonが古く、実際のconfig.jsonが持つフィールドと大幅にズレている。
postman.sh、post.sh、notifier.sh、cocomi-repo-setup.shが参照する全フィールドを反映した
正しいconfig.example.jsonに差し替える。
CLAUDE.mdもファイル構成やconfig.jsonの説明を現状v2.0に合わせて更新する。
...
| ファイル | 操作 |
|---------|------|
| config.example.json | 全体書き換え |
| CLAUDE.md | 全体書き換え |

---

### Step 1/2: config.example.jsonを以下の内容に書き換え

config.example.jsonの中身を **全て削除** して、以下の内容に置き換えてください：

```json
{
  "_comment": "COCOMI Postman 設定ファイル v2.0",
  "_usage": "このファイルをconfig.jsonにコピーして、実際の値を入れてください",
  "_warning": "config.jsonは.gitignoreで除外済み。GitHubにpushしないこと！",

  "default_project": "genba-pro",

  "projects": {
    "genba-pro": {
      "name": "現場Pro設備くん",
      "repo": "akiyamanx/GenbaProSetsubikunN",
      "local_path": "$HOME/GenbaProSetsubikunN"
    },
    "culo-chan": {
      "name": "CULOchanKAIKEIpro",
      "repo": "akiyamanx/CULOchanKAIKEIpro",
      "local_path": "$HOME/CULOchanKAIKEIpro"
    },
    "maintenance-map": {
      "name": "メンテナンスマップ",
      "repo": "akiyamanx/maintenance-map-ap",
      "local_path": "$HOME/maintenance-map-ap"
    },
    "cocomi-postman": {
      "name": "COCOMI Postman",
      "repo": "akiyamanx/cocomi-postman",
      "local_path": "$HOME/cocomi-postman"
    }
  },

  "line": {
    "enabled": true,
    "channel_access_token": "ここにLINE Messaging APIのチャネルアクセストークンを入れる",
    "user_id": "ここにアキヤのLINEユーザーIDを入れる",
    "notify_on": {
...
git commit: "🔧 v2.0 config.example.jsonを実運用フィールドに更新"

### Step 2/2: CLAUDE.mdを以下の内容に書き換え

CLAUDE.mdの中身を **全て削除** して、以下の内容に置き換えてください：
...
git commit: "📝 v1.1 CLAUDE.mdをv2.0現状に合わせて更新"

---

## ✅ 完了条件

1. config.example.jsonが上記の内容に差し替わっていること
2. CLAUDE.mdが上記の内容に差し替わっていること
3. 各ファイルにバージョン情報が入っていること
4. git pushが成功していること
